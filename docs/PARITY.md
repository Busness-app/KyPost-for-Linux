...
| Encrypted database at rest | **Partial** | Mechanism landed 2026-08-22; **not yet switched on**. `Database::open(path, rawKey)` opens an encrypted database via SQLCipher and refuses rather than falling back to plaintext; `main.cpp` still opens unkeyed, so nothing about the shipped behaviour has changed yet. Still to come: key generation and custody (SecureStore, sealed under the PIN when the credential gate is on), migrating an existing plaintext profile, and the interactions with `LocalDataWipe`/HLP/the wipe tripwire. The driver question turned out to be a non-question — Qt's stock QSQLITE plugin drives SQLCipher unchanged, provided the library carries the `libsqlite3.so.0` SONAME the plugin asks for; no vendored Qt driver, no private Qt headers, no custom plugin. |
