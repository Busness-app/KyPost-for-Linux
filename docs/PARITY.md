...
| Replies discarded when they outlive their pairing | **Partial** | Added 2026-08-22. `PairingStore::stillCurrent()` compares the (subscriberId, deviceId) identity captured **before** a request against the one held when its reply lands; `MailRepository::applyRefresh` and `PushRepository::pullOnce` write nothing when it has moved. Deliberately not keyed on `deviceSecret`, which the credential gate rewrites on every lock/unlock. **Still unguarded, same defect class:** `FolderRepository::applyList`/`applyMutation`, `GroupsRepository::applyRefresh`, `ContactSyncRepository::applySync`, downloaded attachments, and the cached PGP compose state. Each writes relay data into a per-device store with no subscriber column, so each can still deposit the previous account's data after the replacement purge. |
