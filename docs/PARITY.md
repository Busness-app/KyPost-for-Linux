...
| Client-side signing and encryption on send | Matched, with one gap | Completed 2026-08-23. A client-custody account on a machine with a usable gpg now sends from the app: the account's own address comes from `/api/pgp/bootstrap` (`suggestedUserIDs[0]`, the same IMAP username the relay authorises the From against), recipient keys come from the relay's discovery ladder and are imported into the user's own GnuPG keyring (AGENTS.md 4b) after checking the fingerprint gpg computes against the one the relay claimed -- in a scratch keyring, so a mismatch never touches the real one. Signing and encryption are one `gpgme_op_encrypt_sign`, RFC 3156 combined mode. To+Cc share one delivery and each Bcc recipient gets their own, so no blind recipient is named in another's key IDs. The real subject travels only as a protected header inside the ciphertext. The Sent copy is encrypted to the sender or **not saved at all** -- a cleartext copy would land on the account's IMAP host, which holds no key. Nothing is sent if any recipient lacks a usable key, and the user is told which one; there is no downgrade to plaintext, because the relay's own fallback works by storing the message on the relay. The webmail hand-off stays for accounts this machine cannot encrypt for. **One gap, deliberate: attachments are REFUSED on an encrypted send** -- this path writes its own MIME and does not build multipart bodies yet, and silently dropping the files would be worse than saying so. Refused in C++, not only in QML. |
