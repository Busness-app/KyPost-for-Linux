...
QList<QByteArray> splitOnBoundary(const QByteArray& body, const QByteArray& boundary)
{
    ...
    while (searchFrom <= body.size()) {
        ...
        if (partStart >= 0) {
            qsizetype end = at;
            // The CRLF (or LF) before a delimiter belongs to the delimiter,
            // not to the part -- RFC 2046 is explicit, and keeping it appends
            // a phantom blank line to every part.
        ...
        // RFC 2046: the delimiter is the boundary followed by optional
        // linear whitespace and then a line break, or by "--" to close.
        // Checking for that terminator is not pedantry -- without it a
        // boundary of "b1" also matches the delimiter "--b11", so a sender
        // who picks colliding boundaries decides where this parser splits,
        // and MIME source ends up rendered to the reader as message text.
        // Found by walking a 40-deep fixture whose boundaries b1 and b11
        // collided; every other test here used boundaries that could not.
        qsizetype cursor = at + delimiter.size();
        while (cursor < body.size() && (body.at(cursor) == ' ' || body.at(cursor) == '\t'))
            ++cursor;

        if (body.mid(cursor, 2) == "--")
            return parts; // closing delimiter; anything after it is epilogue

        const bool terminated =
            cursor >= body.size() || body.at(cursor) == '\r' || body.at(cursor) == '\n';
        if (!terminated) {
            // A longer boundary that merely starts with ours.
            searchFrom = at + delimiter.size();
            continue;
        }

        qsizetype nextLine = body.indexOf('\n', cursor);
        if (nextLine < 0)
            return parts;
